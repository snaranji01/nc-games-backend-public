...
getAllTableNames = async () => {
    const { rows: queryResponse } = await db.query(`SELECT tablename FROM pg_catalog.pg_tables WHERE schemaname='public';`);
    const tableNames = queryResponse.map(responseEl => responseEl.tablename.toString());
    return tableNames;
}

getAllColumnNames = async (tableName) => {
    const { rows: queryResponse } = await db.query(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '${tableName}';`);
    const columnNames = queryResponse.map(responseEl => responseEl.column_name.toString());
    return columnNames;
}
//////////////////////////////////////////////////////////////


exports.checkPrimaryKeyValueExists = async (primaryKeyColumnName, tableName, valueToCheck) => {
    ...
    const { rows: selectIdsResponse } = await db.query(`SELECT ${primaryKeyColumnName} FROM ${tableName};`);
    const existingIds = selectIdsResponse.map(responseEl => responseEl[primaryKeyColumnName].toString());
    ...
    if (existingIds.includes(valueToCheck)) {
        ...
        errorMsgRefObj = {
            ...
            users: `404 Error Not Found: No users with the username provided was found`,
        };
...
exports.checkStringIsPositiveInteger = async (inputString, tableName) => {
